...
module.exports = function update(options, optionalLogger) {
  ...
  if (options.source === os.tmpdir()) {
    ...
    return Promise.reject(
      'Source directory is the Node temp directory. Cowardly refusing to fill up disk with recursive copy.' + JSON.stringify(
        { source: options.source, tmpdir: os.tmpdir() }));
  }
  ...
  return loadConfig(options, { lambda: { name: true, region: true } }).then(function (config) {
    ...
    if (requiresHandlerUpdate) {
      return lambda.updateFunctionConfigurationPromise(
        { FunctionName: lambdaConfig.name, Handler: functionConfig.Handler });
    }
  }).then(function () {
